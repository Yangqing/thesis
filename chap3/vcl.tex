...
\section{Background}
%Category-level visual appearance modeling makes it possible to classify image into a large number of categories. Specifically, the state-of-the-art classification pipeline usually extracts dense feature from local image patches, using either hand-tuned features such as SIFT \cite{lowe1999object} or features learned from certain dictionary learning approaches \cite{olshausen1997sparse,lee2009convolutional}, and spatially pools the local features \cite{lazebnik2006beyond,yang2009linear,boureau2010learning}. Such structures are sometimes repeated in a deep fashion \cite{le2012icml,krizhevsky2012imagenet} to get a vector representation of the input images. Classification methods such as SVMs or logistic regressors are then learned on a set of training images. Algorithms following such a pipeline have exhibited promising performance on a large number of object categories, such as Pascal VOC \cite{pascal} and the ImageNet challenge \cite{imagenet}.

In machine vision, scant attention has been given to the problem of learning a visual concept from a few positive examples as we have defined it. When the problem has been
...
All of these models assume that objects
...
% would be particularly useful, for example, to make a robot caregiver
%be able to interact with a human user to obtain and manipulate objects of
%interest to the human using the human’s preferred terms.

%, and usually assume inputs to be \emph{conceptual}, \ie that people are able to perfectly identify which leaf node of a taxonomy an object should be assigned to based on its appearance. This is a major limitation of these models from the perspective of providing a solution to the problem of word learning with \emph{perceptual} inputs. In addition, this assumption may not accurately model reflect human behavior, as people also perform word learning in a perceptual space that may generate additional uncertainty. Our dataset and proposed algorithms aim to solve this shortcoming by employing object recognition algorithms and providing a joint end-to-end model.

%While it perfectly fits into the classification framework, hut human performance and many real-world tasks do not always occur in such a setting, and a system often does not have prior about where in a hierarchy a concept that is being defined occurs. A practical task as given in Figure \ref{fig:conceptfigure} is how a system decides to which level it should generalize: while ``dogs'' and ``animals'' are all valid descriptions of the objects in (b), the system should not consider other animals as ``Daks''. Addressing such generalization problems is particularly useful, for example, to have a robot caregiver be able to interact with a human user to obtain and manipulate objects of interest to the human using the human’s preferred terms. However, not only are there no methods to our knowledge which accomplish this, there are no machine vision datasets for evaluation.


%Unfortunately, Xu and Tenenbaum \cite{xu2007word} handcrafted their three tested domains to ensure they were psychologically valid. In a first study of forming automated large-scale word learning experiments, Abbott et al. \cite{abbott2012} tested six domains that were automatically generated from WordNet \cite{fellbaum2010wordnet,miller1995wordnet} using images in each concept of each domain from ImageNet \cite{imagenet}. Although Abbott et al.\ automatically generated domains, the set of images from each category were hand constructed. 
...
\begin{figure}
    \centering
    \newcommand{\berryim}[1]{\includegraphics[width=0.055\textwidth]{figs/vcl/imagenet/#1.png}}
    \newcommand{\queryim}[1]{\includegraphics[width=0.055\textwidth]{figs/vcl/imagenet/#1.png}}
